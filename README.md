...
directives that matter against slow-HTTP attacks. Each finding has a `status`:
`missing` (directive absent) or `weak` (present but the value is too permissive
— e.g. `client_header_timeout 600s;` or a per-IP cap of 5000), with severity
and remediation. Directive time values understand units (`10s`, `500ms`, `2m`;
bare HAProxy numbers are treated as milliseconds):
...
It exits non-zero when any gap (missing **or** weak) is at least
`--fail-severity` (default `high`), so it can gate CI/config review. The output
generated by `harden <server>` is guaranteed to pass `audit <server>` with zero
gaps.
